...
namespace RoseLib.Tests
{
    public class InterfaceCompositionTests
    {
        ...
        public void NestedInterfaces()
        {
            ...
            var newNestedInterface1 = "ITestNestedInterface1";
            Regex testRegexNI1 = new Regex(newNestedInterface1);

            var newNestedInterface2 = "ITestNestedInterface2";
            Regex testRegexNI2 = new Regex(newNestedInterface2);

            var newNestedInterface3 = "ITestNestedInterface3";
            Regex testRegexNI3 = new Regex(newNestedInterface3);
            ...
            composer
                ...
                .AddInterface(new Model.InterfaceProperties
                {
                    InterfaceName = newNestedInterface1
                })
                .AddInterface(new Model.InterfaceProperties
                {
                    InterfaceName = newNestedInterface2
                })
                .EnterInterface()
                .AddInterface(new Model.InterfaceProperties
                {
                    InterfaceName = newNestedInterface3
                })
                ;
            ...
            Assert.IsTrue(testRegexU.IsMatch(code));
            Assert.IsTrue(testRegexI.IsMatch(code));
            Assert.IsTrue(testRegexNI1.IsMatch(code));
            Assert.IsTrue(testRegexNI2.IsMatch(code));
            Assert.IsTrue(testRegexNI3.IsMatch(code));

            var interfaceNode = composer
                .StartNavigating()
                .SelectInterfaceDeclaration(newNestedInterface3)
                .State.Peek().CurrentNode;
            Assert.That(interfaceNode!.Parent!.GetType(), Is.EqualTo(typeof(InterfaceDeclarationSyntax)));

            var parentInterfaceNode = interfaceNode!.Parent!;
            Assert.That(parentInterfaceNode!.Parent!.GetType(), Is.EqualTo(typeof(InterfaceDeclarationSyntax)));
        }
